...
import useWebSocketStreamInsom from "@/redux//hooks/useWebSocketStreamInsom";
...
// --- Combined Swipe Pill Logic (Call and Put) ---
function SwipeCallOptionPill({ unqOptPillId,  idx, side, label, ltp, strike,subtitle, onAction , onBuy, onSell, className = "" }) {
    const dispatch = useDispatch();
    const x = useMotionValue(0);
    const [locked, setLocked] = useState(false);
    
    // Quantity state, initialized from mapper or default (1 lot = 75 units)
    const initialLotCount = useMemo(() => {
        const lotCount = optionStrikeQtyMapper.get(`qty_lot_${idx}`) || 1;
        return lotCount;
    }, [idx]);
    const [lotCount, setLotCount] = useState(initialLotCount);
    const quantity = lotCount * 75;

    // Limit Price state, initialized from mapper or current LTP
    const initialPrice = useMemo(() => {
        return optionStrikeQtyMapper.get(`price_${idx}`) || parseFloat(ltp);
    }, [idx, ltp]);
    const [limitPrice, setLimitPrice] = useState(initialPrice);

    const onLimit = (price) => {
        setLimitPrice(price);
        optionStrikeQtyMapper.set(`price_${idx}`, price);
    };
    ...
    */
    const handleDragEnd = (_, info) => {
        if (locked) return;
        const threshold = 90;
        const roundedLimitPrice = roundToNearest5(limitPrice);

        if (info.offset.x > threshold) {
            setJustAction("BUY");
            setOrderStatus("Buy Placed");
            onBuy?.(parseInt(quantity ) , roundToNearest5(limitPrice ) );
            dispatch(placeBuyOrderDummy(quantity, roundedLimitPrice, strike));
        } else if (info.offset.x < -threshold) {
            setJustAction("SELL");
            setOrderStatus("Sell Placed");
             onSell?.(parseInt(quantity ), roundToNearest5(limitPrice ));
            dispatch(placeSellOrderDummy(quantity, roundedLimitPrice, strike));
        }
    ...
    // Simulate updating lot count in mapper when state changes
    useEffect(() => {
        optionStrikeQtyMapper.set(`qty_lot_${idx}`, lotCount);
    }, [lotCount, idx]);

    const minLTP = parseFloat(ltp);
    const maxPrice = minLTP + 400; // Arbitrary max for slider

    return ( <>
     {  (side !== undefined && side === "CE") ?  (
        <div   className={`relative   ${(side === "CALL" || side === "CE" )? ' -right-20' : ' -left-20'} w-[510px] select-none ${className}`}>
            <AnimatePresence>
                {/* Lock Button */}
                <motion.button
                    key="lock-btn"
                    initial={{ scale: 0, x: side === "CALL" ? -10 : 10 }}
                    animate={{ scale: 1, x: 0 }}
                    exit={{ scale: 0, opacity: 0 }}
                    transition={{ type: "spring", stiffness: 500, damping: 30 }}
                    onClick={() => setLocked(!locked)}
                    className={`absolute z-20 rounded-full bg-white border border-zinc-300 p-1 shadow-lg hover:bg-zinc-100 transition-all duration-150
                                ${side === "CALL" ? '-top-2 -right-2' : '-top-2 -left-2'}`}
                >
                    {locked ? <Lock size={14} className="text-red-500"/> : <Unlock size={14} className="text-green-500"/>}
                </motion.button>
            </AnimatePresence>
            ...
            {!locked && ( 
                <div className="absolute inset-0 pointer-events-none flex items-center justify-between px-2 py-2">
                    <motion.div
                        style={{ opacity: opacitySell }}
                        className="flex items-center gap-2 text-red-600 text-[11px] font-semibold"
                    >
                        <ArrowLeft size={14} /> <span>Slide to Sell</span>
                    </motion.div>
                    <motion.div
                        style={{ opacity: opacityBuy }}
                        className="flex items-center gap-2 text-green-700 text-[11px] font-semibold"
                    >
                        <span>Slide to Buy</span> <ArrowRight size={14} />
                    </motion.div>
                </div>
            ...
            >
                ...
                {/* Right side: Slider & Quantity Control */}
                <div className="flex flex-col items-end pl-3">
                    <div className="flex items-center gap-2 text-[10px]">
                        <SidewaysPriceSlider 
                            idx={idx} 
                            min={minLTP > 1 ?  0.5 : 0.05} 
                            max={maxPrice} 
                            step={0.05} 
                    ...
                    <div className="flex flex-col items-center mt-1">
                        <span className="text-[10px] text-zinc-500 font-medium">Lots (Qty: {quantity})</span>
                        <div className="h-[5px] w-full flex items-center justify-center 
                                        bg-grey-400 dark:text-white font-bold text-base mt-2">
                            {lotCount}
                        </div>
                    </div>

                    <ScrollArrows idx={idx} value={lotCount} setValue={setLotCount} />
                </div>
            </motion.div>
            ...
            <AnimatePresence>
                {justAction && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -10 }}
                        className={`absolute -bottom-6 left-1/2 -translate-x-1/2 text-[11px] font-semibold p-1 rounded ${
                            justAction === "BUY" ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                        } flex items-center gap-1 shadow-lg`}
                    >
                        {justAction === "BUY" ? (
                            <><Check size={12} /> Buy Order Placed</>
                        ) : (
                            <><X size={12} /> Sell Order Placed</>
                        )}
                    </motion.div>
                )}
            </AnimatePresence>
...
// --- Combined Swipe Pill Logic (Call and Put) ---
function SwipePutOptionPill({ unqOptPillId, idx, side, label, ltp, strike,subtitle, onAction , onBuy, onSell, className = "" }) {
    const dispatch = useDispatch();
    const x = useMotionValue(0);
    const [locked, setLocked] = useState(false);
    ...
    */
    ...
    return (<>
       { (side !== undefined && side === "PE") ?  (  <div  unqoptpillid={unqOptPillId} className={`relative   ${(side === "PUT" || side === "PE" ) ? ' -right-20' : ' -left-20'} w-[510px] select-none ${className}`}>
            <AnimatePresence>
                {/* Lock Button */}
                <motion.button
                    key="lock-btn"
...
function OptionRow({ unqid,  strikeData, index ,rowptr ,  atmStrikePrice,  onAction  }) {
   // const { strike, call, put, isATM } = strikeData;
     const {  strike,
                                              id,
                                              type,
                                              expiry,
                                              ...
                                              ask,
                                              volume,}  = strikeData;
       let isATM =   (atmStrikePrice !==undefined  && atmStrikePrice.closestKey  ) &&  atmStrikePrice.closestKey == strike ? true : false ; 
       let  subtitle= `LTP ₹${ltp} · Bid ₹${bid} · Ask ₹${ask}` 
       let row = strikeData;
       let isToShow = Array.isArray(rowptr);
       let unqOptid = `${unqid}-${id || index}`

     // closestKey, closestRow, minDiff
    // Common action handler for both call and put
    const handleAction = ({ side, action, qty, price, strike, row }) => {
        console.log(`Action: ${action} ${side} ${strike} @ ${price} x ${qty}`);
        // Dispatch action to Redux (mocked)
        if (action === "BUY") {
            // dispatch(placeBuyOrder(qty, price, strike));
        } else {
            // dispatch(placeSellOrder(qty, price, strike));
        }
    };
    // {/* (isToShow !==false)  && (  */}
    return (
        ...
        <motion.div 
            ...
            className={`grid grid-cols-[1fr_auto_1fr] gap-4 py-3 px-2 transition-all duration-300
                        ${isATM ? 'bg-indigo-50 border-y-2 border-indigo-200 shadow-inner' : 'border-b border-zinc-200'}`}
        >
            {/* CALL Side {type ==="CE"*/}
                <SwipeCallOptionPill
                unqOptPillId = {`${unqOptid}_call`}     
                idx={`call_${strike}`} 
                side={type}
                label="LTP" 
                subtitle = {subtitle} 
            ...
            {/* Strike Price (Center) */}
            <div className={`flex flex-col items-center justify-center min-w-[80px] text-center p-2 rounded-xl transition-all
                            ${isATM ? 'bg-indigo-600 text-white shadow-xl' : 'text-zinc-800'}`}>
                <span className={`text-xs ${isATM ? 'font-light' : 'font-medium'}`}>Strike Price</span>
                <span className="text-2xl font-extrabold">{strike}</span>
            </div>
            
            {/* PUT Side type ==="PE" && */}
            {  <SwipePutOptionPill
                  unqOptPillId = {`${unqOptid}_put`}    
                idx={`put_${strike}`}
                side={type}
                label="LTP"
                 subtitle = {subtitle} 
...
----------------------------------------------------------------------------- */
function OptionRowOLD({  idx ,  row, onAction , strikeData, index  }) {
  const [locked, setLocked] = useState(false);
...
export default function OptionChainTableSideway() {
    const dispatch = useDispatch();
 ...
 /*   // suppose niftyMap already exists (Map<name, valueArray>)
  ...
  return 0;
   ...
   const [ atmStrikePrice , setAtmStrikePricep ] = useState(0 /* mp => {
    
      // First row value (NIFTY 50)
      if(strikeMapInsom !==undefined && strikeMapInsom !==null ){ 
  const firstRow = strikeMapInsom.get("NIFTY 50");
   const targetLtp = parseFloat(firstRow[3]);
     let closestKey = null;
  let closestRow = null;
  let minDiff = Infinity;

  if (!firstRow) {
    console.log("NIFTY 50 row not found in the map");
    const [key, row] =  strikeMapInsom.entries().get(1);
    const premimum = parseFloat(row[3]);
     const diff = Math.abs(premimum - targetLtp);
    if (diff < minDiff) {
      minDiff = diff;
      closestKey = key;
      closestRow = row;
    }
    return  { closestKey, closestRow, minDiff };
  }

 
  for (const [key, row] of strikeMapInsom.entries()) {
    if (key === "NIFTY 50") continue; // skip self
    if (!key.includes("NIFTY")) continue; // skip non-nifty

    const ltp = parseFloat(row[3]);
    if (isNaN(ltp)) continue;

    const diff = Math.abs(ltp - targetLtp);
    if (diff < minDiff) {
      minDiff = diff;
      closestKey = key;
      closestRow = row;
   ...
   return { closestKey , closestRow, minDiff };
    ...
    let def = { closestKey: "NIFTY25093025100CE", closestRow: "25100",minDiff: '422' }
    return def;
}
 ...
 // Mock useSelector for demonstration purposes
  ...
  // State for the selected expiry date
    ...
    const {  optionsMapInsom, strikeMapInsom } = useWebSocketStreamInsom("wss://localhost:8443/",dispatch );
      ...
      {!showModal && <>
                <div className=" w-full bg-zinc-50 sm:bg-white p-1 sm:p-2"> {/* min-h-screen (gap between positon removed)  p-3 sm:p-6  */}
                    ...
                    <div className="space-y-4">
                        {strikeMapInsom  ? (
                             Array.from(strikeMapInsom.entries())
                           ...
                           .map(( [key, value] ,idx ) => { 
                                    //let nameSt= tradeRow[0];
                                    let rawRow=  value; //tradeRow[1];
                                  //  console.log(`iterating map JSX:  ${idx} + ${key} ${JSON.stringify(value)}`)
//console.log(`rawRow ::   ${JSON.stringify(rawRow)}  `);   
                                // const [name, id, timestamp, ltp, , , , bid, ask, , , volume] = rawRow;
                                const [name, rowptr  ] = rawRow;
                                // console.log(`rowptr ::   ${JSON.stringify(rowptr)}`);
                                   const [ index,  id, timestamp, ltp, , , , bid, ask, , , volume] = rowptr
                                  ...
                                  let type = name.slice(-2);
                                   let strike =  name.slice(11,-2);
                                    ...
                                    // <React.Fragment key={`${id}-${strike}-${type}`}>  </React.Fragment>
                                    /*{/*<OptionProvider key={idx}>
                                     <OptionRow   idx={idx} key={key} row={ rowvalue} onAction={handleAction} /> 
                                        {/**</OptionProvider>           rowptr is send to check if it is array    
                                    */
                                return (
                               ...
                               <OptionRow 
                                    ...
                                    unqid={`${idx}_${id}`} 
                                    strikeData={rowvalue} 
                                    atmStrikePrice={ atmStrikePrice}
                                    rowptr={rowptr}    
                                    index={id} onAction={handleAction}
                                />   
                            ...
                            <div className="text-center py-10 text-zinc-500">
                                No option data available for the selected expiry: {selectedExpiry}
                            </div>
                        )}
                    </div>
