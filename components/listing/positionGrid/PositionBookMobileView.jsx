...
export  const  PositionBookMobileView  =({ sortedData, userLogged, handleSort ,getSortIndicator }) => {
  ...
  useEffect( () => {
    if( sortedData ===null || sortedData ===undefined || sortedData ==='') {
      ...
      setMobSorted(CommonConstants.samplePositionDataVersion1);
      console.log("experiment dummy position book  1");
    }
    else if(Array.isArray(sortedData) &&  (sortedData.length== 0) && userLogged ) {
      let fr =  CommonConstants.samplePositionDataVersion1 ; //JSON.parse(CommonConstants.sampleTradeDataVersion1);
      console.log("fr "+JSON.stringify(fr));
      setMobSorted( (prev) => prev = fr );
     
      console.log("experiment dummy position book 2");
    }
...
*/
  ...
  (  Array.isArray(mobSorted) &&  (mobSorted.length > 0) && userLogged  ) ? mobSorted?.map((row, index) => {
    const  profit =   row['realized_profit'] ;
   const tagCls =    (profit <= 0 ? 'position-row-sell' : 'position-row-buy' );
   console.log("tagCls "+tagCls);
   return ( 
...
*/}
 {/*Quantity */}
  <div className={`px-2 text-xs sm:text-right text-center ${tagCls}`}>
    <span className="sm:hidden font-semibold text-gray-500">{/*Qty: */}</span> {row["netQty"]}
  </div>
   
   {/*avgPrice */}
  <div className={`px-2  text-xs text-center sm:text-left ${tagCls}`}>
    <span className="sm:hidden font-semibold text-gray-500">{/*Tm: */}</span> {row["avgPrice"]}
  </div>
  {/*totCh */}
  <div className={`px-2  text-xs text-center sm:text-left ${tagCls}`}>
    <span className="sm:hidden font-semibold text-gray-500">{/*Tm: */}</span> {row["totCh"]}
  </div>
 ...
 {/*actPrf */}
  <div className={`px-2  text-xs sm:text-right text-center ${tagCls}`} >
    <span className="sm:hidden font-semibold text-gray-500">{/*Prc:*/} </span> {row["calPrf"]}
  </div>
 ...
 {/*BUY Value */}
  ...
  {/* realized_profit */}
  <div class={`px-2 text-xs text-center ${tagCls}`}>
