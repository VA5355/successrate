...
export default function   useWebSocketStreamInsom   (url, dispatch)  {
    const ws = useRef(null);
     // const dispatch = useDispatch();

     const [stateOptionMap , setStateOptionsMap ] = useState( (state) => state?.websocket?.symbols);
     const [optionsMapInsom , setOptionsMap ] = useState(null);
    const [strikeMapInsom , setStrikeMap ] = useState(null);
    ...
    useEffect(() => {
        ...
        try { 
            ...
            data.symbollist.forEach(symbol => {
                const name = symbol[0];
                const id = symbol[1];
                console.log(`getOptionsMapFromResponse ${name} ${id}  `)
                console.log(`getOptionsMapFromResponse  symbol ${JSON.stringify(symbol)}    `)
                ...
                // Tag the option with its unique ID and name
                     ...
                     strikeMapSymbol.set (id+"_"+name , symbol)    
                      console.log(`strikeMapSymbol.set ${ id} _  ${ name}  ->  ${JSON.stringify(symbol)} `)
                }
        ...
        ws.current.onopen = () => {
           ...
           // dispatch({ type: 'SET_LOADING', payload: true });
            ...
            const request = {
                method: 'addsymbol',
                symbols: ['NIFTY 50', 'NIFTY25093025200CE', 'NIFTY25093025200PE' , 'NIFTY25093025100CE', 'NIFTY25093025100PE', 'NIFTY25093025300CE' , 'NIFTY25093025300PE'],
            };
                  ...
                  // console.log("optionsMap is a Map.");
                   // Get an array of just the values
                    ...
                    if(valuesArray.length > 0 ){
                     ...
                     console.log("optionsMap is  Map.  BUT EMPTY ");
                  }
                         ...
                         console.log(` response trade `);
                          ...
                          const niftyMap = new Map(
                           ...
                           // suppose niftyMap already exists (Map<name, valueArray>)
                            ...
                            let sortedNiftyymbols =Array.from(sortedNiftyMap.entries())
                            console.log(` sortedNiftyymbols as trade , ${JSON.stringify(sortedNiftyymbols)} `);
                              // setStrikeMap(sortedNiftyMap);
                            setStrikeMap(prev => {
                                const updated = new Map(prev);
                                sortedNiftyMap.forEach((val, key) => {
                                    updated.set(key, val);
                                });
                                return updated;
                                });
                               //  setStrikeMap(Array.from(sortedNiftyMap.entries()));  // FREQUENCY VERY high nothing rendered
                            // setStrikeMap(sortedNiftyMap);
                    ...
                    const formattedSymbols = response.symbollist.map(s => 
                     ...
                     const niftyMap = new Map(
                      ...
                      let stateSymbols =Array.from(niftyMap.entries())
                           ...
                           console.log('sortedNiftyMap from symbolist:', JSON.stringify([...sortedNiftyMap.entries()]));
                           // console.log(` strikeMap, ${JSON.stringify(niftyMap.entries())} `)
     ...
     return { optionsMapInsom, strikeMapInsom };
};